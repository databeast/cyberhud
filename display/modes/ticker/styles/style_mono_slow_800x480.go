...
// Bespoke BuildFn for the polished monochrome slow-refresh 800×480 e-ink style.
func monoSlow800x480Build(snap source.TickerSnapshot, pol source.Policy, ctx style.StyleContext, d def) style.ViewData {
	hints := ctx.Hints()
	...
	// Step 4 (Row assembly): extract text from directives; ignore ScrollOffset and StripSprites.
	lines := make([]string, len(snap.Directives))
	for i, d := range snap.Directives {
		lines[i] = d.Text
	}

	// Empty state: single "(no feed)" row at TierLarge with full pipeline applied.
	if len(lines) == 0 {
		emptyText := "(no feed)"
