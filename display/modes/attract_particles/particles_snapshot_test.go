...
// snapshotOutputDir is the persistent directory where particles snapshot PNGs are written.
var snapshotOutputDir = filepath.Join("snapshots")

// categoryFromStyleName derives the testsnapshot.DisplayCategory from a particles
// style's name prefix.
...
// TestParticlesPNGSnapshots enumerates all registered particles styles and renders each
// through the full production pipeline via the snapshottest framework.
func TestParticlesPNGSnapshots(t *testing.T) {
	styles := SnapshotStyles()
	if len(styles) == 0 {
		t.Fatal("particlesRegistry contains zero styles")
	}
	...
	for _, s := range styles {
		...
		t.Run(s.Name(), func(t *testing.T) {
			...
			pngPath := testsnapshot.RenderSnapshot(t,
				testsnapshot.WithMode("attract_particles"),
	...
	if err := testsnapshot.WriteGalleryFragmentFromPaths(snapshotOutputDir, "attract_particles", pngPaths); err != nil {
