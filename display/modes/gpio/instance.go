...
func init() {
	displaymodes.RegisterFactory("gpio", newInstance)
}

// instance implements displaymodes.ModeInstance for the GPIO mode.
...
func newInstance() displaymodes.ModeInstance {
	return &instance{
		hints: initialHints(),
		mgr:   source.GPIOManager(), // self-sourced from package singleton
	}
}

func (i *instance) ID() string { return "gpio" }

func (i *instance) Activate() {
	if i.mgr != nil {
		source.SetGPIOManager(i.mgr)
	}
}

func (i *instance) Deactivate() {
	source.SetGPIOManager(nil)
}
...
func (i *instance) BuildView() style.ViewData {
	pins := i.pins()
	vd := BuildView(pins, i.hints, displaymodes.Warnings)
