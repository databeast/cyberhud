...
func TestTestFontsPNGSnapshots(t *testing.T) {
	t.Cleanup(func() { nowFunc = time.Now })

	renderDiagnosticSnapshots(t, "testfonts", func() {
		nowFunc = time.Now
	}, func() {
...
func renderDiagnosticSnapshots(t *testing.T, mode string, reset func(), preRender func()) {
	t.Helper()
	cases := []struct {
		name     string
		width    int
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			pngPath := testsnapshot.RenderSnapshot(t,
				...
				testsnapshot.WithReset(reset),
				testsnapshot.WithPreRender(preRender),
			)
	...
	if err := testsnapshot.WriteGalleryFragmentFromPaths(snapshotOutputDir, mode, pngPaths); err != nil {
