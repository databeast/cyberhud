...
// buildMonoSlowItemsOnly renders a compact e-paper friendly serial layout.
// It keeps the explicit per-style wrappers while centralizing the shared logic.
func buildMonoSlowItemsOnly(snap source.Snapshot, p source.Policy, ctx style.StyleContext, _ def, defaultRows int) style.ViewData {
	hints := ctx.Hints()
