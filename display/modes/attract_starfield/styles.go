...
// starfieldRegistry is the per-mode StyleRegistry for the starfield display mode.
var starfieldRegistry = func() *style.StyleRegistry[source.Snapshot, source.Policy] {
	return style.NewRegistry[source.Snapshot, source.Policy](
...
func registeredStyleNames() []string {
	styles := starfieldRegistry.Enumerate()
...
func resolveIsEink(hints textlayout.TextHints) bool {
	s, _ := style.ResolveStyle(starfieldRegistry, hints, "attract_starfield", "")
	if s == nil {
		return false
	}
	reqs := s.Requirements()
