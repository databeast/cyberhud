...
import (
	...
	"github.com/databeast/cyberhud/display/modes/attract_starfield"
	"github.com/databeast/cyberhud/display/modes/testsnapshot"
	"github.com/databeast/cyberhud/display/style"
)

// snapshotOutputDir is the persistent directory where starfield snapshot PNGs are written.
var snapshotOutputDir = filepath.Join("..", "snapshots")

// categoryFromStyleName derives the testsnapshot.DisplayCategory from a starfield
// style's name prefix.
...
// TestStarfieldPNGSnapshots enumerates all registered starfield styles and renders each
// through the full production pipeline via the snapshottest framework.
func TestStarfieldPNGSnapshots(t *testing.T) {
	styles := attract_starfield.SnapshotStyles()
	if len(styles) == 0 {
		t.Fatal("starfieldRegistry contains zero styles")
	}
	...
	for _, s := range styles {
		...
		t.Run(s.Name(), func(t *testing.T) {
			...
			pngPath := testsnapshot.RenderSnapshot(t,
				testsnapshot.WithMode("attract_starfield"),
				...
				testsnapshot.WithReset(func() {
					attract_starfield.ResetSnapshotState()
				}),
				testsnapshot.WithPreRender(func() {
					attract_starfield.SetPolicy(attract_starfield.DefaultPolicy())
				}),
			)
	...
	if err := testsnapshot.WriteGalleryFragmentFromPaths(snapshotOutputDir, "attract_starfield", pngPaths); err != nil {
